...
import { Music, VolumeX, MapPin, Gift, Copy, Check, Heart, X, PlayCircle, Mail } from 'lucide-react';
  ...
  return (
    <div className="ornament-line" aria-hidden="true">
      <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
        <path d="M7 0L8.75 5.25L14 7L8.75 8.75L7 14L5.25 8.75L0 7L5.25 5.25L7 0Z" fill={color} />
      </svg>
    </div>
  );
}
...
function BankLogo({ name, url }) {
  ...
  if (n.includes('mandiri'))
    return <svg width="72" height="20" viewBox="0 0 120 32"><text x="0" y="24" fontFamily="Arial" fontWeight="bold" fontSize="20" fill="#003d79">mandiri</text></svg>;
...
function HeroLeft({ settings, bride, groom }) {
  const bgStyle = settings.photo_hero
    ? { backgroundImage: `url(${settings.photo_hero})`, backgroundSize: 'cover', backgroundPosition: 'center' }
    : { background: 'linear-gradient(135deg, #3D1A18 0%, #7A231E 50%, #3D1A18 100%)' };

  const dateFormatted = settings?.hero_date
    ? new Date(settings.hero_date + 'T00:00:00').toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).toUpperCase()
    : 'MINGGU, 6 APRIL 2025';
    ...
    <div className="split-left" style={bgStyle}>
      ...
      <div className="hero-text" style={{ paddingBottom: '80px' }}>
        <motion.p
          ...
          style={{ fontFamily: 'var(--font-sans)', fontSize: '0.6rem', letterSpacing: '0.35em', textTransform: 'uppercase', color: 'rgba(253,248,243,0.7)', marginBottom: '12px' }}
        >
        ...
        <motion.h1
          ...
          style={{ fontFamily: 'var(--font-script)', fontSize: 'clamp(3.5rem, 7vw, 5.5rem)', fontWeight: 400, lineHeight: 1.2, textShadow: '2px 4px 12px rgba(0,0,0,0.5)', marginBottom: '16px' }}
        >
          {bride?.nickname || 'Debby'} <span style={{ fontSize: '0.65em', opacity: 0.8 , color: '#fff'}}>&amp;</span> {groom?.nickname || 'Alam'}
        </motion.h1>
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 1, delay: 1.2 }}
          style={{ fontFamily: 'var(--font-sans)', fontSize: '0.62rem', fontWeight: 600, letterSpacing: '0.2em', color: 'rgba(253,248,243,0.75)' }}
        >
          {dateFormatted}
        </motion.p>
      </div>
...
function CoverPanel({ onOpen, guestName, bride, groom, settings }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 1, height: '100%', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      {/* Top decoration */}
      ...
      <Reveal>
        <div style={{
          width: '200px', height: '280px', borderRadius: '80px 80px 80px 80px',
          margin: '0 auto 24px',
          border: '3px solid rgba(153,44,37,0.5)',
          overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center',
          background: 'linear-gradient(135deg, #3D1A18, #5C2420)',
          boxShadow: '0 12px 40px rgba(153,44,37,0.3), 0 0 0 6px rgba(153,44,37,0.1)',
        }}>
          {settings?.photo_story ? (
            <img src={settings.photo_story} alt="Couple" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          ) : (
      ...
      {/* Date */}
      <Reveal delay={0.1}>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.62rem', letterSpacing: '0.25em', color: 'rgba(253,248,243,0.5)', marginBottom: '12px' }}>
          {dateStr}
        </p>
      </Reveal>
      ...
      {/* Names */}
      <Reveal delay={0.2}>
        <h1 className="shimmer-text" style={{ fontFamily: 'var(--font-script)', fontSize: 'clamp(3rem, 8vw, 3.8rem)', fontWeight: 400, lineHeight: 1.1, marginBottom: '4px' }}>
          {bride?.nickname || 'Debby'}
        </h1>
        <p className="shimmer-text" style={{ fontFamily: 'var(--font-display)', fontSize: '1.4rem', fontStyle: 'italic', margin: '8px 0', opacity: 0.9 }}>&amp;</p>
        <h1 className="shimmer-text" style={{ fontFamily: 'var(--font-script)', fontSize: 'clamp(3rem, 8vw, 3.8rem)', fontWeight: 400, lineHeight: 1.1 }}>
          {groom?.nickname || 'Alam'}
        </h1>
      </Reveal>
      ...
      {/* Guest info */}
      <Reveal delay={0.3}>
        <div style={{ marginTop: '32px', borderTop: '1px solid rgba(153,44,37,0.3)', paddingTop: '24px' }}>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(253,248,243,0.45)', marginBottom: '6px' }}>Kepada Yth.</p>
          <p style={{ fontFamily: 'var(--font-serif)', fontSize: '1.05rem', fontWeight: 600, color: '#FAF6F1', marginBottom: '24px' }}>
    ...
    <div style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: '#1a0a09' }}>
      ...
      <AnimatePresence>
        ...
        >
          ...
          <h2 className="shimmer-text" style={{ fontFamily: 'var(--font-script)', fontSize: 'clamp(4rem, 10vw, 6rem)', fontWeight: 400, lineHeight: 1.3, textShadow: '2px 4px 16px rgba(0,0,0,0.6)', marginBottom: '0' }}>
            {bride?.nickname || 'Debby'}
          </h2><br></br>
          <p className="shimmer-text" style={{ fontFamily: 'var(--font-display)', fontSize: '2.2rem', fontStyle: 'italic', margin: '10px 0' }}>&amp;</p>
         <br></br> <h2 className="shimmer-text" style={{ fontFamily: 'var(--font-script)', fontSize: 'clamp(4rem, 10vw, 6rem)', fontWeight: 400, lineHeight: 1.3, textShadow: '2px 4px 16px rgba(0,0,0,0.6)', marginBottom: '0' }}>
            {groom?.nickname || 'Alam'}
          </h2>

          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.68rem', fontWeight: 600, letterSpacing: '0.2em', color: 'rgba(253,248,243,0.7)', marginTop: '24px' }}>
  ...
  return (
    <div className="section-frame" style={{ position: 'relative' }}>
      ...
      <div className="inset-card card-ivory">
        ...
        <Reveal delay={0.1}>
          <p style={{ fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontSize: '0.92rem', lineHeight: 1.9, color: 'rgba(253,248,243,0.85)', maxWidth: '300px', margin: '0 auto 20px' }}>
            &ldquo;{settings?.quote_text || 'Dan di antara tanda-tanda kebesaran-Nya ialah Dia menciptakan pasangan-pasangan untukmu...'}&rdquo;
          </p>
        </Reveal>
        <Reveal delay={0.2}>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.7rem', color: 'rgba(253,248,243,0.6)', letterSpacing: '0.15em', fontWeight: 600 }}>
...
function PersonCard({ person, photo_url, delay = 0, isBride = false }) {
  ...
  const igHandle = person.instagram_url ? person.instagram_url.split('/').filter(Boolean).pop() : '';
  return (
    <Reveal delay={delay}>
      <div style={{ textAlign: 'center', marginBottom: '40px', position: 'relative' }}>
        ...
        <div style={{
          ...
          margin: '0 auto 20px', border: '2px solid rgba(253,248,243,0.3)',
          overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        <p style={{ fontFamily: 'var(--font-serif)', fontSize: '0.9rem', fontWeight: 600, color: 'rgba(253,248,243,0.85)', marginBottom: '4px' }}>
          {person.full_name}
        </p>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.72rem', color: 'rgba(253,248,243,0.5)', marginBottom: '2px' }}>
  ...
  return (
    <div className="section-frame" style={{ position: 'relative' }}>
      ...
      <div className="inset-card card-dark">
        ...
        style={{ position: 'absolute', top: '5px', left: '255px', zIndex: 1 }}
      >
        <motion.img 
        ...
        />
          ...
          <p className="section-label" style={{ color: 'rgba(253,248,243,0.6)' }}>Mempelai</p>
          <h2 style={{ fontFamily: 'var(--font-script)', fontSize: '3.2rem', color: '#FAF6F1', marginBottom: '8px', fontWeight: 400 }}>
            We Are Getting Married!
          </h2>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.8rem', color: 'rgba(253,248,243,0.8)', lineHeight: 1.8, maxWidth: '280px', margin: '0 auto 48px', opacity: 0.85 }}>
        ...
        <Reveal delay={0.3}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', justifyContent: 'center', margin: '4px 0 32px' }}>
            <div style={{ height: '1px', width: '60px', background: 'rgba(253,248,243,0.2)' }} />
            <span style={{ fontFamily: 'var(--font-display)', fontSize: '2.4rem', color: 'rgba(253,248,243,0.8)', fontStyle: 'italic' }}>&amp;</span>
            <div style={{ height: '1px', width: '60px', background: 'rgba(253,248,243,0.2)' }} />
          </div>
        </Reveal>

        <PersonCard person={groom} photo_url={settings?.photo_groom} delay={0.4} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="section-frame" style={{ position: 'relative' }}>
      ...
      <motion.div
        ...
        style={{ position: 'absolute', top: '-125px', left: '-125px', zIndex: 99 }}
      >
        <motion.img 
      ...
      <motion.div
        ...
        style={{ position: 'absolute', bottom: '-105px', right: '-150px', zIndex: 99 }}
      >
        <motion.img 
          src="/assets/main/bunga.png" 
      ...
      <div className="inset-card card-ivory" style={{ position: 'relative', zIndex: 2 , textAlign: 'center'}}>
        <Reveal>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.65rem', letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(253,248,243,0.6)', marginBottom: '16px', fontWeight: 600 }}>
            Menghitung Hari
          </p>
          <h2 style={{ fontFamily: 'var(--font-script)', fontSize: '3.2rem', color: '#FAF6F1', fontWeight: 400, marginBottom: '32px' }}>
            Save The Date
          </h2>
          <div className="countdown-grid">
            {[['d', 'Days'], ['h', 'Hours'], ['m', 'Minutes'], ['s', 'Seconds']].map(([k, label]) => (
              <div key={k} className="countdown-box" style={{ borderColor: 'rgba(253,248,243,0.2)' }}>
                <div className="num" style={{ color: '#FAF6F1' }}>{pad(t[k])}</div>
                <div className="unit" style={{ color: 'rgba(253,248,243,0.7)' }}>{label}</div>
              </div>
            ))}
          </div>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.7rem', letterSpacing: '0.2em', color: 'rgba(253,248,243,0.8)', marginTop: '24px', fontWeight: 600 }}>
            {settings?.hero_date
              ? new Date(settings.hero_date + 'T00:00:00').toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).toUpperCase()
              : 'MINGGU, 6 APRIL 2025'}
          </p>
          ...
          <a
            ...
            className="open-btn"
            style={{ marginTop: '24px', display: 'inline-flex', padding: '12px 24px', fontSize: '0.8rem' }}
          >
...
function EventCard({ event, delay = 0 }) {
  ...
  return (
    <Reveal delay={delay}>
      <div className="event-arch" style={{ background: 'rgba(253,248,243,0.05)', borderColor: 'rgba(253,248,243,0.15)', boxShadow: 'none' }}>
        {/* Icon */}
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '20px' }}>
          <Heart size={32} color="#FAF6F1" style={{ opacity: 0.85 }} />
        </div>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.62rem', letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(253,248,243,0.6)', marginBottom: '8px' }}>
          Insya Allah akan dilaksanakan pada
        </p>
        <h3 style={{ fontFamily: 'var(--font-script)', fontSize: '2.4rem', color: '#FAF6F1', fontWeight: 400, marginBottom: '16px' }}>
          {event.type}
        </h3>
        <p style={{ fontFamily: 'var(--font-serif)', fontSize: '0.92rem', fontWeight: 700, color: '#FAF6F1', marginBottom: '4px' }}>
          {dateFormatted}
        </p>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.8rem', color: 'rgba(253,248,243,0.8)', marginBottom: '20px' }}>
          {event.start_time} – {event.end_time} WIB
        </p>
        <div style={{ height: '1px', background: 'rgba(253,248,243,0.1)', margin: '0 20px 20px' }} />
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '8px' }}>
          <MapPin size={18} color="#FAF6F1" />
        </div>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.88rem', fontWeight: 600, color: '#FAF6F1', marginBottom: '4px' }}>
          {event.location_name}
        </p>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.76rem', color: 'rgba(253,248,243,0.7)', lineHeight: 1.6, marginBottom: '20px' }}>
          {event.address}
        </p>
        {event.map_url && (
          <a href={event.map_url} target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '9px 22px', border: '1.5px solid rgba(253,248,243,0.3)', borderRadius: '50px', background: 'transparent', color: '#FAF6F1', fontFamily: 'var(--font-sans)', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.05em', cursor: 'pointer', textDecoration: 'none' }}>
    ...
    <div className="section-frame" style={{ position: 'relative' }}>
      ...
      <div className="inset-card card-dark">
        <Reveal>
          ...
          <h2 style={{ fontFamily: 'var(--font-script)', fontSize: '3.2rem', color: '#FAF6F1', fontWeight: 400, marginBottom: '40px' }}>Acara Pernikahan</h2>
        </Reveal>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {events.map((e, i) => <EventCard key={i} event={e} delay={0.1 * i} />)}
        </div>
      </div>
    </div>
  );
    ...
    <div className="section-frame" style={{ position: 'relative' }}>
      ...
      <div className="inset-card card-ivory">
        <Reveal>
          ...
          <h2 style={{ fontFamily: 'var(--font-script)', fontSize: '3.2rem', color: '#FAF6F1', fontWeight: 400, marginBottom: '40px' }}>Our Love Story</h2>
        </Reveal>
          ...
          {stories.map((s, i) => (
            <Reveal key={i} delay={0.1 * i}>
              <div style={{ display: 'flex', gap: '16px', marginBottom: '32px', alignItems: 'flex-start' }}>
                {s.image_url && (
                  <div style={{ width: '72px', height: '72px', flexShrink: 0, borderRadius: '12px', backgroundImage: `url(${s.image_url})`, backgroundSize: 'cover', backgroundPosition: 'center', border: '2px solid rgba(253,248,243,0.2)' }} />
                )}
                <div>
                  <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.62rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(253,248,243,0.6)', marginBottom: '4px', fontWeight: 600 }}>{s.date}</p>
                  <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.05rem', fontWeight: 700, color: '#FAF6F1', marginBottom: '6px' }}>{s.title}</h3>
                  <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.8rem', color: 'rgba(253,248,243,0.8)', lineHeight: 1.8 }}>{s.description}</p>
                </div>
              </div>
            </Reveal>
          ))}
      ...
      <div className="inset-card card-dark" style={{ borderRadius: '24px' }}>
        <Reveal>
          <div style={{ background: 'rgba(253,248,243,0.05)', border: '1px solid rgba(253,248,243,0.1)', borderRadius: '16px', padding: '40px 24px' }}>
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.7rem', fontWeight: 600, color: '#FAF6F1', marginBottom: '16px', letterSpacing: '0.2em', textTransform: 'uppercase' }}>
              Doa Pengantin
            </p>
            <p style={{ fontFamily: 'var(--font-serif)', fontSize: '1.6rem', color: '#FAF6F1', marginBottom: '20px', direction: 'rtl', lineHeight: 2 }}>
              بَارَكَ اللَّهُ لَكَ وَبَارَكَ عَلَيْكَ وَجَمَعَ بَيْنَكُمَا فِي خَيْرٍ
            </p>
            <p style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: '0.92rem', color: 'rgba(253,248,243,0.75)', lineHeight: 1.9 }}>
              &ldquo;Semoga Allah memberkahimu dan memberkahi atas kamu, dan mengumpulkan kalian berdua dalam kebaikan.&rdquo;
            </p>
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.7rem', color: 'rgba(253,248,243,0.45)', marginTop: '16px', letterSpacing: '0.1em' }}>
  ...
  return (
    <div className="section-frame">
      <div className="inset-card card-ivory" style={{ borderRadius: '24px' }}>
        <Reveal>
          <p className="section-label" style={{ color: 'rgba(253,248,243,0.6)' }}>Wedding Gift</p>
          <h2 style={{ fontFamily: 'var(--font-script)', fontSize: '3.2rem', color: '#FAF6F1', fontWeight: 400, marginBottom: '12px' }}>Kirim Kado</h2>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.8rem', color: 'rgba(253,248,243,0.8)', marginBottom: '36px', maxWidth: '280px', margin: '0 auto 36px', lineHeight: 1.8, opacity: 0.85 }}>
            Kehadiran Anda adalah hadiah terbesar bagi kami.
          </p>
        </Reveal>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {gifts.filter(g => !g.is_address).map((a, i) => (
            <Reveal key={i} delay={0.1 * i}>
              <div style={{ background: 'rgba(253,248,243,0.05)', borderRadius: '16px', padding: '20px', border: '1px solid rgba(253,248,243,0.1)', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
                  <div style={{ textAlign: 'left' }}>
                    <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', fontWeight: 600, color: '#FAF6F1', marginBottom: '2px' }}>{a.bank_name}</p>
                    <p style={{ fontFamily: 'var(--font-serif)', fontSize: '1.05rem', fontWeight: 700, color: '#FAF6F1', letterSpacing: '0.05em' }}>{a.account_number}</p>
                    <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.75rem', color: 'rgba(253,248,243,0.7)' }}>a.n <strong>{a.account_name}</strong></p>
                  </div>
                  <BankLogo name={a.bank_name} url={a.logo_url} />
                </div>
                <button onClick={() => doCopy(a.account_number, i)} className="gift-btn">
                  {copied === i ? <><Check size={15} /> Tersalin!</> : <><Copy size={15} /> Salin Nomor</>}
                </button>
              </div>
            </Reveal>
          ))}
          ...
          {gifts.filter(g => g.is_address).map((a, i) => (
            <Reveal key={`addr-${i}`} delay={0.3}>
              <div style={{ background: 'rgba(253,248,243,0.05)', borderRadius: '16px', padding: '20px', border: '1px solid rgba(253,248,243,0.1)', textAlign: 'center' }}>
                <Gift size={32} color="rgba(253,248,243,0.8)" style={{ margin: '0 auto 12px', opacity: 0.7 }} />
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.88rem', fontWeight: 600, color: '#FAF6F1', marginBottom: '8px' }}>Kirim Kado Fisik</p>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.78rem', color: 'rgba(253,248,243,0.7)', lineHeight: 1.7, marginBottom: '16px' }}>{a.text_content}</p>
                <button onClick={() => doCopy(a.text_content, `addr-${i}`)} className="gift-btn">
                  {copied === `addr-${i}` ? <><Check size={15} /> Tersalin!</> : <><Copy size={15} /> Salin Alamat</>}
                </button>
              </div>
            </Reveal>
          ))}
...
function GuestbookSection({ guestName }) {
  ...
  const [form, setForm] = useState({ name: guestName || '', attendance: 'attending', message: '' });
  ...
  return (
    <div className="section-frame">
      <div className="inset-card card-ivory" style={{ borderRadius: '24px' }}>
        <Reveal>
          <p className="section-label" style={{ color: 'rgba(253,248,243,0.6)' }}>Buku Tamu</p>
          <h2 style={{ fontFamily: 'var(--font-script)', fontSize: '3.2rem', color: '#FAF6F1', fontWeight: 400, marginBottom: '12px' }}>
            Ucapan & Doa
          </h2>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.8rem', color: 'rgba(253,248,243,0.8)', marginBottom: '32px', maxWidth: '280px', margin: '0 auto 32px', lineHeight: 1.8, opacity: 0.85 }}>
            Tinggalkan pesan dan konfirmasi kehadiran Anda untuk mempelai.
          </p>
        </Reveal>

        <Reveal delay={0.1}>
          <form onSubmit={handleSubmit} style={{ background: 'rgba(253,248,243,0.05)', borderRadius: '16px', padding: '24px 20px', border: '1px solid rgba(253,248,243,0.1)', textAlign: 'left', marginBottom: '32px' }}>
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontFamily: 'var(--font-sans)', fontSize: '0.75rem', color: '#FAF6F1', marginBottom: '8px', fontWeight: 600 }}>Nama Anda</label>
              <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} required style={{ width: '100%', padding: '12px 16px', borderRadius: '8px', background: 'rgba(26, 10, 9, 0.4)', border: '1px solid rgba(253,248,243,0.2)', color: '#FAF6F1', fontFamily: 'var(--font-sans)', fontSize: '0.85rem' }} placeholder="Masukkan nama..." />
            </div>
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontFamily: 'var(--font-sans)', fontSize: '0.75rem', color: '#FAF6F1', marginBottom: '8px', fontWeight: 600 }}>Kehadiran</label>
              <select value={form.attendance} onChange={e => setForm({...form, attendance: e.target.value})} style={{ width: '100%', padding: '12px 16px', borderRadius: '8px', background: 'rgba(26, 10, 9, 0.4)', border: '1px solid rgba(253,248,243,0.2)', color: '#FAF6F1', fontFamily: 'var(--font-sans)', fontSize: '0.85rem' }}>
                <option value="attending" style={{ color: '#1A0A09' }}>Ya, Saya akan hadir</option>
                <option value="not_attending" style={{ color: '#1A0A09' }}>Maaf, tidak bisa hadir</option>
              </select>
            </div>
            <div style={{ marginBottom: '24px' }}>
              <label style={{ display: 'block', fontFamily: 'var(--font-sans)', fontSize: '0.75rem', color: '#FAF6F1', marginBottom: '8px', fontWeight: 600 }}>Ucapan & Doa</label>
              <textarea value={form.message} onChange={e => setForm({...form, message: e.target.value})} required rows={4} style={{ width: '100%', padding: '12px 16px', borderRadius: '8px', background: 'rgba(26, 10, 9, 0.4)', border: '1px solid rgba(253,248,243,0.2)', color: '#FAF6F1', fontFamily: 'var(--font-sans)', fontSize: '0.85rem', resize: 'vertical' }} placeholder="Tuliskan ucapan dan doa..."></textarea>
            </div>
            <button type="submit" disabled={isSubmitting} className="open-btn" style={{ width: '100%', padding: '14px', borderRadius: '8px', opacity: isSubmitting ? 0.7 : 1 }}>
        ...
        <Reveal delay={0.2}>
          <div style={{ maxHeight: '300px', overflowY: 'auto', paddingRight: '4px', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {comments.map((c, i) => (
              <div key={i} style={{ padding: '16px', borderRadius: '12px', background: 'rgba(253,248,243,0.03)', border: '1px solid rgba(253,248,243,0.08)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                  <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', fontWeight: 700, color: '#FAF6F1', margin: 0 }}>{c.guest_name}</p>
                  <span style={{ fontSize: '0.65rem', padding: '2px 8px', borderRadius: '20px', background: c.attendance === 'attending' ? 'rgba(76, 175, 80, 0.2)' : 'rgba(244, 67, 54, 0.2)', color: c.attendance === 'attending' ? '#81c784' : '#e57373', border: `1px solid ${c.attendance === 'attending' ? 'rgba(76, 175, 80, 0.3)' : 'rgba(244, 67, 54, 0.3)'}` }}>
                    {c.attendance === 'attending' ? 'Hadir' : 'Tidak Hadir'}
                  </span>
                </div>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.8rem', color: 'rgba(253,248,243,0.8)', margin: '0 0 8px', lineHeight: 1.6 }}>{c.message}</p>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.65rem', color: 'rgba(253,248,243,0.4)', margin: 0 }}>{formatDate(c.created_at)}</p>
              </div>
            ))}
          </div>
        </Reveal>
...
export default function WeddingTemplate({ guestName }) {
  ...
  useEffect(() => {
    if (bride && groom) document.title = `${bride.nickname} & ${groom.nickname} — The Wedding`;
  }, []);
  ...
  return (
    <div className="split-layout">
      ...
      <div className="split-right">
        <AnimatePresence>
          {!isOpen ? (
            <CoverPanel
              key="cover"
              onOpen={() => { setIsOpen(true); setMusicPlaying(true); }}
              guestName={guestName || 'Tamu Undangan'}
              bride={bride} groom={groom} settings={settings}
            />
          ) : (
            <motion.div key="content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
              ...
              <MainCoupleSection bride={bride} groom={groom} settings={settings} />
              <CountdownSection settings={settings} bride={bride} groom={groom} />
      ...
      {isOpen && (
        <button className={`music-btn ${musicPlaying ? 'playing' : ''}`} onClick={() => setMusicPlaying(!musicPlaying)} aria-label="Toggle Music">
